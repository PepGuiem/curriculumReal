...
var gw = 0;
var gh = 0;
caballero.src = document.querySelector(".caballero").src;
caballeroRight.src = document.querySelector(".caballero-right").src;
caballeroLeft.src = document.querySelector(".caballero-left").src;
cofre.src = document.querySelector(".chest").src;
paredes.src = document.querySelector(".wall").src;
suelo.src = document.querySelector(".street").src;
...
play.addEventListener("click", function() {
    ...
    whoLevelIts();
    playing = true;
    if (level != 1) document.querySelector(".light-content2").style.display = "none";
});
document.addEventListener("keydown", function(event) {
    let tecla = event.key.toLowerCase();
    if (tecla == "w" && playing) {
        if (posPerson[0] - 1 > -1 && levelMap[posPerson[0] - 1][posPerson[1]] != "X") {
            if (levelMap[posPerson[0] - 1][posPerson[1]] == "#") document.querySelector(".next-level").style.display = "block";
            else movimentJugadorWS(posPerson, levelMap, 1, caballero, 0);
        }
    } else if (tecla == "a" && playing) {
        if (posPerson[1] - 1 > -1 && levelMap[posPerson[0]][posPerson[1] - 1] != "X") {
            if (levelMap[posPerson[0]][posPerson[1] - 1] == "#") document.querySelector(".next-level").style.display = "block";
            else movimentJugadorAD(posPerson, levelMap, 1, caballeroRight, 0);
        }
    } else if (tecla == "s" && playing) {
        if (posPerson[0] + 1 < vertical && levelMap[posPerson[0] + 1][posPerson[1]] != "X") {
            if (levelMap[posPerson[0] + 1][posPerson[1]] == "#") document.querySelector(".next-level").style.display = "block";
            else movimentJugadorWS(posPerson, levelMap, -1, caballeroLeft, 10);
        }
        ...
        if (posPerson[1] + 1 < horizontal && levelMap[posPerson[0]][posPerson[1] + 1] != "X") {
            if (levelMap[posPerson[0]][posPerson[1] + 1] == "#") {
                if (level == 7) document.querySelector(".projects-buttton").style.display = "block";
                else document.querySelector(".next-level").style.display = "block";
            } else movimentJugadorAD(posPerson, levelMap, -1, caballeroLeft, 10);
        }
    }
});
...
function pintarFila(i, y) {
    ...
    for(let j = 0; j < horizontal; j++){
        if (levelMap[i][j] == "X") contexto.drawImage(paredes, x, y, gw, gh);
        else {
            contexto.drawImage(suelo, x, y, gw, gh);
            if (levelMap[i][j] == "!") contexto.drawImage(caballero, x, y, gw + 10, gh + 15.6);
            else if (levelMap[i][j] == "#") contexto.drawImage(cofre, x, y, gw + 10, gh);
        }
